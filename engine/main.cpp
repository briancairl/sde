...
int main(int argc, char** argv)
{
  ...
  auto front_movement_sprite_sheet = Image::load("/home/brian/dev/assets/red_sprite/Top Down/Front Movement.png", {.flags = {.flip_vertically = true}});
  SDE_ASSERT_TRUE(front_movement_sprite_sheet.has_value());

  auto back_movement_sprite_sheet = Image::load("/home/brian/dev/assets/red_sprite/Top Down/Back Movement.png", {.flags = {.flip_vertically = true}});
  SDE_ASSERT_TRUE(back_movement_sprite_sheet.has_value());

  auto side_movement_sprite_sheet = Image::load("/home/brian/dev/assets/red_sprite/Top Down/Side Movement.png", {.flags = {.flip_vertically = true}});
  SDE_ASSERT_TRUE(side_movement_sprite_sheet.has_value());


  auto front_movement_atlas = assets.textures.create(*front_movement_sprite_sheet);
  SDE_ASSERT_TRUE(front_movement_atlas.has_value());

  auto back_movement_atlas = assets.textures.create(*back_movement_sprite_sheet);
  SDE_ASSERT_TRUE(back_movement_atlas.has_value());

  auto side_movement_atlas = assets.textures.create(*side_movement_sprite_sheet);
  SDE_ASSERT_TRUE(side_movement_atlas.has_value());


  auto front_movement_frames = assets.tile_sets.create(
    *front_movement_atlas,
    TileSetSliceUniform{
      .tile_size_px = {64, 64},
  ...
  SDE_ASSERT_TRUE(front_movement_frames.has_value());

  auto back_movement_frames = assets.tile_sets.create(
    *back_movement_atlas,
    TileSetSliceUniform{
      .tile_size_px = {64, 64},
      .tile_orientation_x = TileOrientation::kNormal,
  ...
  SDE_ASSERT_TRUE(back_movement_frames.has_value());

  auto right_movement_frames = assets.tile_sets.create(
    *side_movement_atlas,
    TileSetSliceUniform{
      .tile_size_px = {64, 64},
      .tile_orientation_x = TileOrientation::kNormal,
  ...
  SDE_ASSERT_TRUE(right_movement_frames.has_value());

  auto left_movement_frames = assets.tile_sets.create(
    *side_movement_atlas,
    TileSetSliceUniform{
      .tile_size_px = {64, 64},
  ...
  SDE_ASSERT_TRUE(left_movement_frames.has_value());
  ...
  RenderResources sprite_render_resources;
  sprite_render_resources.shader = (*sprite_shader_or_error);
  sprite_render_resources.textures[0] = (*front_movement_atlas);
  sprite_render_resources.textures[1] = (*back_movement_atlas);
  sprite_render_resources.textures[2] = (*side_movement_atlas);
  ...
  AnimatedSprite front_movement_animated_sprite{*front_movement_frames, 5.0F, AnimatedSprite::Mode::kLooped};
  AnimatedSprite back_movement_animated_sprite{*back_movement_frames, 5.0F, AnimatedSprite::Mode::kLooped};
  AnimatedSprite right_movement_animated_sprite{*right_movement_frames, 5.0F, AnimatedSprite::Mode::kLooped};
  AnimatedSprite left_movement_animated_sprite{*left_movement_frames, 5.0F, AnimatedSprite::Mode::kLooped};

  AnimatedSprite* animation = &front_movement_animated_sprite;
  ...
  app.spin([&](const auto& window)
  {
    ...
    direction.setZero();
    animation = &front_movement_animated_sprite;

    if (window.keys.isDown(KeyCode::kA))
    {
      direction[0] = -kMoveRate;
      animation = &left_movement_animated_sprite;
      //attributes.world_from_camera(0, 2) -= time_delta * kMoveRate;
    }
    ...
    {
      direction[0] = +kMoveRate;
      animation = &right_movement_animated_sprite;
      //attributes.world_from_camera(0, 2) += time_delta * kMoveRate;
    }
    ...
    {
      direction[1] = +kMoveRate;
      animation = &back_movement_animated_sprite;
      //attributes.world_from_camera(1, 2) += time_delta * kMoveRate;
    }
    ...
    animation->update(attributes.time);

    position += direction * time_delta;

    window_target_or_error->refresh(Black());
    if (auto render_pass_or_error = RenderPass::create(*window_target_or_error, *renderer_or_error, assets, attributes, sprite_render_resources); render_pass_or_error.has_value())
    {
      animation->draw(*render_pass_or_error, {position - sde::Vec2f{0.5, 0.5}, position + sde::Vec2f{0.5, 0.5}});
    }
