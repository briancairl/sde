...
std::ostream& operator<<(std::ostream& os, ComponentError error)
{
  switch (error)
  {
    SDE_OSTREAM_ENUM_CASE(ComponentError::kInvalidHandle)
    SDE_OSTREAM_ENUM_CASE(ComponentError::kComponentLibraryInvalid)
    SDE_OSTREAM_ENUM_CASE(ComponentError::kComponentLibraryMissingFunction)
    SDE_OSTREAM_ENUM_CASE(ComponentError::kComponentAlreadyLoaded)
  }
  return os;
}
...
expected<void, ComponentError> ComponentCache::reload(ComponentData& component)
{
  ...
  {
    SDE_LOG_ERROR() << "ComponentLibraryInvalid: " << SDE_NAMED(component.library);
    return make_unexpected(ComponentError::kComponentLibraryInvalid);
  }

  if (!component.io.reset(library_ptr->lib))
  {
    SDE_LOG_ERROR() << "ComponentLibraryMissingFunction: " << SDE_NAMED(component.library);
...
expected<ComponentData, ComponentError> ComponentCache::generate(const asset::path& path)
{
  ...
  {
    SDE_LOG_ERROR() << "ComponentLibraryInvalid: " << SDE_NAMED(path) << ", " << SDE_NAMED(library_or_error.error());
    return make_unexpected(ComponentError::kComponentLibraryInvalid);
  }
