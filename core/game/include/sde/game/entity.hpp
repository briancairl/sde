...
{
public:
  EntityCreation(ComponentCache& components, Registry& reg, EntityData& entity) :
...
public:
  ...
  decltype(auto) make_entity(dependencies deps, AttachComponentsT attach_components)
  {
    ...
    if (value_or_error.has_value())
    {
      ...
      EntityCreation entity_creation{deps.get<ComponentCache>(), deps.get<Registry>(), entity_itr->second.value};
